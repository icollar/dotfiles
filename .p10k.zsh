# Generated by Powerlevel10k configuration wizard on 2023-01-03 at 20:36 GMT.
...
() {
  ...
  function my_git_formatter() {
    ...
    local       meta='%93F' # white foreground
    local      clean='%232F' # black foreground
    local   modified='%B%232F' # black foreground
    local  untracked='%B%232F' # black foreground
    local conflicted='%B%232F' # red foreground
  ...
  typeset -g POWERLEVEL9K_STATUS_ERROR_SIGNAL_FOREGROUND=250
  ...
  function prompt_dir2 () {
    ...
    for part ($(echo "${CUR//\// }")) {
      ...
      if [[ -d "/$(joined)/${part}/.git" ]]; then
        p10k segment -b 0 -f 250 -t "$(joined2)/%F{255}%B${part}%b%f"
        p10k segment -b 76 -f 232 -t ""
        parts=()
        GIT=1
        continue
      fi
    ...
    if [[ ${#parts} -gt 0 ]]; then
      parts[-1]="%B%F{255}${part:-~}"
    fi
    p10k segment -b 0 -f 250 -t "$(joined2)"
  }
